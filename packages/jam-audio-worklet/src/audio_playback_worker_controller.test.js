...
test('ended hold expiry emits ended without another refill tick', () => {
  const messages = [];
  let intervalCallback = null;
  ...
  assert.ok(messages.some(
    (message) =>
      message.type === 'diagnostics-event' &&
      message.event.type === 'ended-emission-state' &&
      message.event.action === 'hold-started',
  ));
  ...
  assert.ok(messages.some(
    (message) =>
      message.type === 'diagnostics-event' &&
      message.event.type === 'ended-emission-state' &&
      message.event.action === 'emitted',
  ));
  assert.ok(messages.some((message) => message.type === 'ended'));
});

test('ended hold expiry suppresses ended when pending gapless bytes arrive', () => {
  const messages = [];
  let intervalCallback = null;
  let timeoutCallback = null;
  let nowValue = 0;
  ...
  sharedState[2] = 0;
  intervalCallback();
  controller.preloadNext(new Uint8Array([2, 3, 4]), { sampleRate: 48000 });
  ...
  assert.ok(messages.some(
    (message) =>
      message.type === 'diagnostics-event' &&
      message.event.type === 'ended-emission-state' &&
      message.event.action === 'suppressed-pending-gapless',
  ));
  assert.equal(messages.some((message) => message.type === 'ended'), false);
});
