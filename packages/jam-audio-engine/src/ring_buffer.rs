...
impl PcmRingBuffer {
    ...
    pub fn push_interleaved(
        ...
        if !interleaved_samples.len().is_multiple_of(self.layout.channels) {
...
mod tests {
    ...
    fn layout_exposes_shared_memory_sizing() {
        let layout = RingBufferLayout::new(DEFAULT_FRAME_CAPACITY, 2);
    ...
    fn ring_buffer_pushes_and_pops_in_frame_order() {
        let mut buffer = PcmRingBuffer::new(4, 2);
    ...
    fn ring_buffer_wraps_and_reuses_capacity() {
        let mut buffer = PcmRingBuffer::new(3, 2);
